...
4. 部署配置默认使用 `free` 实例，不要求购买常驻计算资源。请注意免费服务空闲 15 分钟后会休眠，下一位玩家首次打开时通常需要等待约一分钟唤醒。
...
- 免费实例会在空闲 15 分钟后休眠；新访问会自动唤醒，网站不会因此被永久删除。
- 没有玩家时不会自动开始游戏或调用 OpenAI。
...
在 Render 服务的 **Settings** 中把 Instance Type 从 `free` 升级为付费实例即可取消空闲休眠。是否升级由你决定，当前仓库配置不会要求付费。
