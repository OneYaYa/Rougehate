...
宣传片以一场绝境反击呈现核心玩法：自定义战斗身份、穿过敌群的虫洞冲刺、七神赐福、敌群合围与濒死压迫、自然语言 AI 锻造特写、密集敌潮反攻、武器蓝图重构，以及“遮天幼星群”“万伏雷鳗”“事件视界”三件具名后期成型武器。全部战斗画面均由浏览器中的实时 Canvas 玩法录制，配乐、逐字键盘音与转场音效为本地程序化原创。
...
- **无模板 AI 攻击异变**：每第 4 次升级，玩家可以自己描述改造，也可以让 AI 根据战况推荐；系统只生成一个完成度更高的结果，并以 5 秒动态预览展示真实攻击行为。
- **三套独立升级演出**：普通遗物是短促的同步印记；星神祝福会先以重锤落砧声、领域色爆闪和神祇立绘全屏降临，再翻出该星神专属的三项随机祝福；AI 武器则把当前武器投影成动态蓝图，依次完成扫描、拆解与协议锁定。
...
2. 从普通星兽群中升级；星神来访时会先显现身份，再从同一位星神的三项随机祝福中选择一项，每局最多结识 4 位宇宙赐福者。
3. 每第 4 次升级进入一次“武器异梦”，改造已有武器的攻击形态；锻造前可以查看循环实战预览。
...
无需修改个人主页或使用第三方网页部署平台。可以让自己的 Windows 电脑通过公网 IP 直接提供游戏与 Python API，具体见 [WINDOWS_IP_DEPLOY.md](WINDOWS_IP_DEPLOY.md)。自有 Linux 服务器的 Docker/Caddy 方案见 [DEPLOY.md](DEPLOY.md)。API Key 只保存在服务器 `.env` 中，绝不能写入浏览器代码或 Git 仓库。
...
导演模式位于 `?trailer=1`，只用于确定性实机录制，不会影响正常游戏。当前分镜约 54 秒，采用“爆点冷开场 → 身份定义 → 早期生存 → 七神赐福阵列 → 具体赐福效果 → 多方向追逃绝境 → AI 锻造输入 → 密集敌潮逆转 → 武器蓝图重构 → 三件具名成型武器”的递进结构。开场敌群会沿两次虫洞冲刺的路径和落点布置，位移始终穿过战场压力区。七神片段集中展示新版原创立绘、**7 位独特的宇宙神祇 / 63 项专属赐福**，随后用“太阳的指纹”明确展示击杀残焰与范围灼烧效果；异变片段会直接展示当前武器的实时轮廓与扫描、拆解、锁定流程。绝境段由不同方向、不同速度与不同职责的敌群切断撤离路线，玩家会持续走位和冲刺，不再使用同步收缩的规则圆环。输入阶段保留 AI 锻造标题、输入框、字数与提交按钮，并让逐字键盘音与构想文字同步。反攻段使用五组不规则敌潮展现清场，终局则给“遮天幼星群”“万伏雷鳗”“事件视界”各自约五秒的具名展示。结尾品牌落版会复用启动页的定制字标与 AI 武器编译阵列，保证游戏首屏和宣传片使用同一套视觉语言。
