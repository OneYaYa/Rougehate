...
宣传片以一场绝境反击呈现核心玩法：自定义战斗身份、移动与虫洞冲刺、遗物/赐福选择、敌群合围与濒死压迫、自然语言 AI 锻造、生成武器清场、武器异变，以及五武器终局对抗 Boss。全部战斗画面均由浏览器中的实时 Canvas 玩法录制，配乐与转场音效为本地程序化原创。
...
导演模式位于 `?trailer=1`，只用于确定性实机录制，不会影响正常游戏。当前分镜约 43 秒，采用“爆点冷开场 → 身份定义 → 早期生存 → 构筑选择 → 濒死合围 → 输入愿望 → AI 武器逆转清场 → 武器异变 → 终局 Boss”的递进结构；战斗段会驱动角色持续横穿、变向和冲刺。最终成片使用本地生成的原创 BGM 与转场音效。
