...
A Windows PC can host the game and Python API directly through a public IP; see [Windows IP Deployment](WINDOWS_IP_DEPLOY.md). For a self-managed Linux server with Docker and Caddy, see [Deployment](DEPLOY.md). Keep the API key in the server's `.env` file—never in browser code or Git.
